...
Английский каталог переводов лежит в `locale/en/LC_MESSAGES/` — и `.po`,
и **скомпилированный `.mo`**. `.mo` намеренно закоммичен: на образе сборки
может не быть `gettext`, и тогда `compilemessages` не отработает, а английская
версия молча уедет в русский текст. Поэтому в Build Command его нет.

Если правите интерфейсные строки, локально нужен `gettext`:

```
apt-get install -y gettext
python manage.py makemessages -l en --no-location --no-wrap
# перевести пустые msgstr в locale/en/LC_MESSAGES/django.po
python manage.py compilemessages -l en
```

Содержимое (тексты страниц, названия служб, праздники) переводится **не
здесь, а в админке**: у каждого такого поля есть вкладки RU и EN. Пустое
английское поле молча показывает русский текст — это штатный фолбэк
modeltranslation, а не ошибка.
