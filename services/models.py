...
# Повторяющееся, а не однократное: «По воскресеньям», а не «В воскресенье».
# Полоса под первым экраном говорит о постоянном ритме, и единственное число
# там читалось бы как обещание конкретного дня.
WEEKDAY_HABITUAL = [
    _("По понедельникам"), _("По вторникам"), _("По средам"), _("По четвергам"),
    _("По пятницам"), _("По субботам"), _("По воскресеньям"),
]

# Клетка полосы под первым экраном — это одна крупная строка «Вс 09:00».
# Полное «По воскресеньям» туда не встаёт, а сокращение читается сразу.
WEEKDAY_SHORT = [
    _("Пн"), _("Вт"), _("Ср"), _("Чт"), _("Пт"), _("Сб"), _("Вс"),
]
...
# Воскресенье и суббота — опоры недели, а не просто два из семи дней: на них
# стоят литургия и всенощная. Обычная сортировка по номеру дня начинается с
# понедельника, и приход с правилами на каждый день показал бы в полосе
# понедельник, вторник и среду, потеряв главное.
_RHYTHM_PRIORITY = {6: 0, 5: 1}


def weekly_rhythm(limit=3):
    """Постоянный ритм недели для полосы под первым экраном.

    Берётся из недельного шаблона, который секретарь и так заполняет ради
    генерации расписания, — а не угадывается по уже проставленным дням.
    Угадывание ломается именно тогда, когда важнее всего: на Страстной и
    святках расписание нарочно не похоже на обычную неделю.
    """
    rules = (
        RecurringServiceRule.objects.filter(is_active=True)
    ...
    for rule in rules:
        ...
        if not items:
            continue
        # В клетку помещается одна служба, и это должна быть главная службa
        # дня, а не первая по времени: воскресенье с часами в 08:40 и
        # литургией в 09:00 обязано показать литургию. День служб идёт к
        # своей главной — часы перед литургией, вечерня перед утреней, — так
        # что последняя по порядку и есть та, ради которой приходят.
        main = items[-1]
        rows.append({
            "weekday": rule.weekday,
            "label": WEEKDAY_HABITUAL[rule.weekday],
            "short_label": WEEKDAY_SHORT[rule.weekday],
            "main": {"name": main.service_type.name, "time": main.time},
            "services": [
                {"name": i.service_type.name, "time": i.time} for i in items
            ],
        })
    rows.sort(key=lambda r: (_RHYTHM_PRIORITY.get(r["weekday"], 2), r["weekday"]))
    return rows[:limit]
...
class ServiceDay(models.Model):
    ...
    def weekday_short(self):
        ...
        return WEEKDAY_SHORT[self.date.weekday()]
